...
(function () {
  ...
  function gitLine(esc, g) {
    ...
    const dirty = g.dirty ? `<span class="text-yellow">${g.dirty} changed</span>` : `<span class="text-green">clean</span>`;
    const wt = g.worktrees > 1 ? ` · ${g.worktrees} worktrees` : "";
  ...
  function envRows(esc, envs) {
    ...
    return `<table class="studio-table text-xs"><thead><tr><th>env</th><th>url</th><th>host</th><th>deploy</th><th>status</th><th>control</th></tr></thead><tbody>` +
      envs.map(e => `<tr>
        <td>${esc(e.name)}</td>
        <td>${e.url ? `<a class="text-blue" href="${esc(e.url)}" target="_blank" rel="noopener">${esc(e.url)}</a>` : `<span class="text-subtext">none</span>`}</td>
        <td>${esc(e.host || "")}</td>
        <td>${esc(e.deploy || "")}</td>
  ...
  function appCard(esc, a) {
    const repo = a.repo || {};
    const remote = repo.remote ? `<a class="text-blue" href="${esc(remoteHref(repo.remote))}" target="_blank" rel="noopener">${esc(repo.remote)}</a>` : "";
  ...
  function domainRows(esc, domains) {
    ...
    return `<table class="studio-table text-xs"><thead><tr><th>host</th><th>app</th><th>env</th><th>status</th><th>control</th></tr></thead><tbody>` +
      domains.map(d => `<tr>
        <td><a class="text-blue" href="${esc(d.url)}" target="_blank" rel="noopener">${esc(d.host)}</a></td>
  ...
  async function mount(root, slug, helpers) {
    ...
    catch (e) { root.innerHTML = `<p class="text-xs text-yellow">studio unavailable: ${esc(e.message)}</p>`; return; }
    ...
    root.innerHTML = `<div class="studio-grid">
      <section>
        <h2 class="font-pixel text-xs text-green mb-2">Library (${doc.counts.apps})</h2>
        ${errors}
      ...
      <section>
        <h2 class="font-pixel text-xs text-green mb-2">Domains (${doc.counts.domains})</h2>
